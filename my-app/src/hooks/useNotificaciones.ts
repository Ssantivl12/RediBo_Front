...
export function useNotifications() {
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_APIBACK}/api/notificaciones/dropdown-notificaciones/${userId}`);
  ...
  const fetchUnreadCount = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_APIBACK}/api/notificaciones/notificaciones-no-leidas/${userId}`);
      const data = await response.json();
  ...
  const markAsRead = useCallback(async (notificationId: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_APIBACK}/api/notificaciones/notificacion-leida/${notificationId}/${userId}`,
        { method: 'PUT' }
      );
